...
class HydroDS {
    ...
    static_assert(BucketCapacity >= 16 && BucketCapacity <= 4096,
                  "BucketCapacity must be in [16, 4096]");

    // -------------------------------------------------------------------------
    //  Constants
    // -------------------------------------------------------------------------
    static constexpr int C        = BucketCapacity;
    static constexpr double EPS_HIGH = 0.85;   // Flow trigger threshold
    static constexpr double EPS_LOW  = 0.50;   // Flow target threshold
